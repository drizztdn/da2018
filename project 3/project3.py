...
def clean():
    ...
    d = pd.DataFrame(columns=['ReserveLevel','Installment','Region','Platform','Channel','Edition','RelativeWeek'])
    ...
    for u in df['Installment'].value_counts().index.tolist():
        for m in df['RelativeWeek'].value_counts().index.tolist():
            # print(m)
            d.loc[x] = [df['ReserveLevel'][np.logical_and(df['RelativeWeek'] == m, df['Installment'] == u)].sum(),
                        u, 'All', 'All', 'All', 'All', m]
            x = x + 1
            for i in df['Region'].value_counts().index.tolist():
                # print(i)
                d.loc[x] = [df['ReserveLevel'][np.logical_and(np.logical_and(df['Region'] == i, df['RelativeWeek'] == m), df['Installment'] == u)].sum(),
                            u, i, 'All', 'All', 'All', m]
                x = x + 1
                for j in df['Platform'].value_counts().index.tolist():
                    # print(j)
                    d.loc[x] = [df['ReserveLevel'][np.logical_and(
                    ...
                    for k in df['Channel'].value_counts().index.tolist():
                        # print(k)
                        d.loc[x] = [df['ReserveLevel'][np.logical_and(np.logical_and(
                        ...
                        for l in df['Edition'].value_counts().index.tolist():
                            # print(l)
                            # print(df['ReserveLevel'][np.logical_and(np.logical_and(
                            #     np.logical_and(np.logical_and(df['Region'] == i, df['Platform'] == j), df['Channel'] == k),
                            #     df['Edition'] == l), df['RelativeWeek'] == m)].sum())
                            d.loc[x] = [df['ReserveLevel'][np.logical_and(np.logical_and(np.logical_and(
                                np.logical_and(np.logical_and(df['Region'] == i, df['Platform'] == j), df['Channel'] == k),
...
res = sm.ols(formula="ReserveLevel ~ RegionOrd+PlatformOrd+ChannelOrd+EditionOrd+RelativeWeek", data=d).fit()

print(res.summary())

fig, ax = plt.subplots()
# ax.plot(d['RelativeWeek'], d['ReserveLevel'])
for u in d['Installment'].value_counts().index.tolist():
    ax.plot(d['RelativeWeek'][np.logical_and(np.logical_and(
        ...
        d['Edition'] == 'All'), d['Installment'] == u)], d['ReserveLevel'][np.logical_and(np.logical_and(
        np.logical_and(np.logical_and(d['Region'] == 'All', d['Platform'] == 'All'), d['Channel'] == 'All'),
